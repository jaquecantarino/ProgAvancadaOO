...
function max0(xs: number[]) : number | undefined { // o | representa a união de dois tipos.
    if (xs.length == 0) return undefined; //aqui ele já define o retorno em casos de vetor vazio, já retorno o undefined.
    let cand = xs[0];
    for(const x of xs) 
        if (x>cand) { 
            cand=x;
...
//jeito 1:
function maxSt(xs: string[], key: (s:string) => number) : string | undefined {
    if (xs.length == 0 ) return undefined;
    let cand = xs[0];
    for (const s of xs)
    if (key(s) > key(cand)) cand =s;
    return cand;
}
//???//
/*function square(x: number): number { return x*x; }
...
//jeito 2:
function maxSt2<A>(xs: Array<A>, key: (s:A) => number) : A | undefined {
    if (xs.length == 0 ) return undefined;
    let cand = xs[0];
    for (const s of xs)
    if (key(s) > key(cand)) cand =s;
